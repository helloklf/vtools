...
class fragment_booster : Fragment() {
    ...
    lateinit internal var packageInfos: List<ApplicationInfo>
    ...
    internal fun LoadList() {
        ...
        for (i in packageInfos.indices) {
            if (packageInfos[i].sourceDir.indexOf("/system") == 0)
                continue
            val item = HashMap<String, Any>()
            val d = packageInfos[i].loadIcon(packageManager)
            item.put("icon", d)
            val pkgName = packageInfos[i].packageName.toLowerCase()
