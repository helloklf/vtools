...
object CommonCmds {
    ...
    const val DisableChanger =
            "if [ -f '/sys/class/power_supply/battery/battery_charging_enabled' ]; then echo 0 > /sys/class/power_supply/battery/battery_charging_enabled;\n" +
            "elif [ -f /sys/class/power_supply/battery/constant_charge_current_max ]; then chmod 664 /sys/class/power_supply/battery/constant_charge_current_max;echo 0 > /sys/class/power_supply/battery/constant_charge_current_max;fi;\n" +
                    "else echo 1 > /sys/class/power_supply/battery/input_suspend; fi;setprop vtools.bp 1;\n"
    const val ResumeChanger =
            "if [ -f /sys/class/power_supply/battery/constant_charge_current_max ]; then echo 3000000 > /sys/class/power_supply/battery/constant_charge_current_max;fi;\n" +
                    ...
                    "if [ -f '/sys/class/power_supply/battery/battery_charging_enabled' ]; then echo 1 > /sys/class/power_supply/battery/battery_charging_enabled; else echo 0 > /sys/class/power_supply/battery/input_suspend; fi;\n" +
                    "setprop vtools.bp 0;\n"
