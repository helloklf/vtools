...
var app = new Vue({
  ...
  methods: {
    ...
    onSessionClick(session) {
      ...
      // 流畅度 (帧率 ≥40 的比例)
      detail.fluency = '' + ((fpsData.filter(it => it >= 40).length) / fpsData.length * 100).toFixed(1);
      let maxTemp = 0
      ...
      detail.highTempRatio = '' + ((temperatureData.filter(it => it >= 45).length) / temperatureData.length * 100).toFixed(1);
