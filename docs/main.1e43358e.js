    ...
    animationTo: function animationTo(to) {
      ...
      if (this.animation.interval < 0) {
        this.animation.interval = window.setInterval(function () {
          var v = Math.abs(_this.animation.current - _this.animation.to);
          var sqrt = parseInt(Math.sqrt(v, 3)) / 2;

          if (v <= sqrt) {
            window.clearInterval(_this.animation.interval);
            _this.animation.interval = -1;
            _this.animation.current = _this.animation.to;
          } else if (_this.animation.to > _this.animation.current) {
            _this.animation.current += sqrt;
          } else {
            _this.animation.current -= sqrt;
          }

          _this.$refs.phone.style.transform = "translateX(".concat(_this.animation.current, "px)");
        }, 4);
      }
    }
  },
